(function ($) {
    ...
    $(document).on("click", ".city_edit", function () {
        ...
        let seo_description = $(this).attr('data-seodescription');
          let seo_keywords = $(this).attr('data-seo_keywords');
        ...
        if (city_thumb) {
            ...
            $('#preview_thumb').attr('src', `https://via.placeholder.com/120x150?text=NSN`);
        }

        if (city_banner) {
            $('#preview_banner').attr('src', `/uploads/${city_banner}`);
        } else {
            $('#preview_banner').attr('src', `https://via.placeholder.com/372x150?text=NSN`);
        }
...
function placeMap() {
    ...
    let place_lat = parseFloat($('#city_lat').val()) || 20.5937;
    let place_lng = parseFloat($('#city_lng').val()) || 78.9629;
