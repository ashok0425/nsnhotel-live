...
function SEOMeta($title = '', $description = '', $keywords = '', $image = null, $canonical = '', $type = 'website')
{
...
function getCalender($months,$hotel_id)
{


     $db_data =   Calendar::where('hotel_id',$hotel_id)->where('date', '=', $months)->groupBy('date')->first();
     return $db_data;
}
function getCalendes($months,$hotel_id)
{
     $db_datas =   Calendar::where('hotel_id',$hotel_id)->where('date', '=',$months)->groupBy('room_id')->get();
     foreach($db_datas  as $rooms_slug){
         if($db_datas){
       $model = Room::where('hotel_id',$hotel_id)->where('id',$rooms_slug->room_id)->first();
         $db_data =   Calendar::where('room_id',$rooms_slug->room_id)->where('date','=',$months)->groupBy('room_id')->first();
     
       $st_room = round($db_data->price_one_person * $db_data->discount_percentage / 100);
       $nd_room = round($db_data->price_one_person * $db_data->discount_percentage / 100);
       $rd_room = round($db_data->price_one_person * $db_data->discount_percentage / 100);
       $model->onepersonprice = $db_data->price_one_person- $st_room;
       $model->twopersonprice = $db_data->price_two_person - $nd_room;
       $model->threepersonprice = $db_data->price_three_person - $rd_room;
       $model->before_discount_price = $db_data->price_one_person;
       $model->save();
     }
     }
