...
//              }
    ...
    public function getLogin(Request $request) {
        ...
        if($data) {
            if($data['mobile'] == '9876543210'){
                     $user   =   User::where('phone_number',$data['mobile'])->first();    
            }
            else{
                  $user   =   User::where('phone_number',$data['mobile'])->first(); 
    ...
    public function cancelBooking(Request $request){
        $user=$this->getUserByApiToken($request);
        $booking = Booking::where('user_id',$user->id)->where('id',$request->id)->update(['status'=>0]);  
        $this->whatsapp_cancel('977'.$booking->phone_number, $booking->name);
        $this->whatsapp_cancel('91'.$booking->phone_number, $booking->name);
