...
<div class="card custom-border-radius-0">
    <a class="card-body p-2" href="{{ route('place_detail', ['slug' => $place->slug]) }}">
        <div class="image_warpper">
            <img src="" class="img-fluid lazy  border card_image_height custom-border-radius-10"
                data-src="{{ getImageUrl($place->thumb) }}" alt="{{ $place->name }}" class="" />
        </div>
                ...
                <div class="w-25 text-right">
                    ...
                    @if ($place->roomsData->first()->discount_percent)
                        ...
                        <p class="custom-fs-16 custom-fw-600 custom-text-gray-2 mb-1 pb-0"><del>
                                @price_formatter($place->roomsData->first()->before_discount_price)
                            </del></p>
                    @endif
                </div>
            </div>
            ...
            <div class="name_price d-flex mb-1 pb-0">
                ...
                <div class="w-50 text-right">
                    ...
                    <p class="custom-fs-20 custom-text-dark custom-fw-600 pt-0 mt-0 mb-0 pt-0">
                        @price_formatter($onepersonprice)
                        <small class="text-success custom-fs-14">{{ $place->roomsData->first()->discount_percent }}% off</small>
                    </p>
...
{{-- rating  --}}
            <div class="rating_wrapper d-flex mt-0 pt-0">
                ...
                <div class="w-50 text-right">
                    <small class="custom-text-12 custom-text-gray-2 custom-fw-600 mt-0 pt-0">1 room per night</small>
                </div>
            </div>
