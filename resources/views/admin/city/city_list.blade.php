...
@else
                                ...
                                <table id="example1" class="display full-width table-striped table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($cities as $city)
                                            <tr>
                                                <td>{{ $city->id }}</td>
                                                <td><img class="city_list_img" src="{{ getImageUrl($city->thumb) }}"
                                                        alt="city thumb"></td>
                                                <td>{{ $city->name }} </td>
                                                <td>{{ $city->description }}</td>
                                                <td>{{ $city->priority }}</td>
                                                <td>
                                                    <a href=" https://nsnhotels.com/city/{{ $city->slug }}">
                                                        https://nsnhotels.com/city/{{ $city->slug }}</a>
                                                </td>
                                                <td>
                                                    <input type="checkbox" class="js-switch city_status"
                                                        data-id="{{ $city->id }}" {{ isChecked($city->status, 1) }} />
                                                    <br>
                                                    <br>
                                                    @if ($city->popular == 1)
                                                        <a href="{{ route('admin_list-as-popular', ['city' => $city->id, 'status' => 0]) }}"
                                                            class="btn btn-xs btn-success">Popular</a>
                                                    @else
                                                        <a href="{{ route('admin_list-as-popular', ['city' => $city->id, 'status' => 1]) }}"
                                                            class="btn btn-xs btn-danger">Popular</a>
                                                    @endif
                                                ...
                                                <td>
                                                    <button type="button" class="btn btn-warning city_edit"
                                                        ...
                                                        data-name="{{ $city->name }}" data-slug="{{ $city->slug }}"
                                                        data-intro="{{ $city->intro }}"
                                                        data-description="{{ $city->description }}"
                                                        data-thumb="{{ $city->thumb }}" data-banner="{{ $city->banner }}"
                                                        ...
                                                        data-popular="{{ $city->popular }}">Edit
                                                    </button>
                                                    <form class="d-inline"
                                                        action="{{ route('admin_city_delete', $city->id) }}"
                                                        method="POST">
                                                        @method('DELETE')
                                                        @csrf
                                                        <button type="button"
                                                            class="btn btn-danger city_delete">Delete</button>
                                                    </form>

                                                    <button type="button" class="btn btn-primary btn-sm "
                                                        data-toggle="modal" data-target="#pricechangemodal"
                                                        id="price_modify" data-id="{{ $city->id }}">
                                                        Modify price
                                                    </button>
                                                </td>
                                            </tr>
    ...
    <div class="modal fade" id="pricechangemodal" tabindex="-1" role="dialog" aria-labelledby="pricechangemodalLabel"
        aria-hidden="true">
                ...
                <div class="modal-body">
                    <form action="{{ route('admin_modify_price') }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="percent" placeholder="Enter percent" class="form-control"
                                required>
                        </div>
                        <div class="form-group">
