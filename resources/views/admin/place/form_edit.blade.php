    ...
    <div class="tab-content">
                        ...
                        @foreach($languages as $index => $language)
                            ...
                            <div class="tab-pane fade show {{$index !== 0 ?: "active"}}" id="language_{{$language->code}}" role="tabpanel" aria-labelledby="home-tab">
                                <div class="form-group">
                                    <label for="place_name">Name Of The Hotel <small>({{$language->code}})</small>: *</label>
                                    <input type="text" class="form-control" name="{{$language->code}}[name]" value="{{$trans['name']}}" placeholder="What the name of hotel" autocomplete="off" {{$index !== 0 ?: "required"}}>
                                </div>
                                <div class="form-group">
                                    <label for="name">Description <small>({{$language->code}})</small>: *</label>
                                    <textarea type="text" class="form-control tinymce_editor" name="{{$language->code}}[description]" rows="6" >{{$trans['description']}}</textarea>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="form-group col-md-6">
                    ...
                    <select class="form-control js-example-basic-multiple" id="user_id" name="user_id" required>
                        @foreach($users as $user)
                            <option value="{{$user->id}}" {{isSelected($user->id, $place->user_id)}}>{{$user->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="checkbox row">
                ...
                @foreach($amenities as $item)
                    <div class="col-md-4 mb-10">
                        <label class="p-0"><input type="checkbox" class="flat" name="amenities[]" value="{{$item->id}}" {{isChecked($item->id, $place->amenities)}} @if(!$place->amenities)  @if (in_array($item->id,$amen)) checked @endif   @endif> {{$item->name}}</label>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div id="location">
            ...
            <div class="form-group mt-4">
                ...
                <input type="text" class="form-control" id="place_address" name="airport" value="{{$place->airport}}" placeholder="Chaudhary Charan Singh Int. Airport 8.7 KM" maxlength="12" autocomplete="off">
                <label for="place_address">Distance from Railway Station: </label>
                <input type="text" class="form-control" id="place_address" name="railway_station" value="{{$place->railway_station}}" placeholder="Lucknow Jn. Station 250 M" maxlength="12" autocomplete="off" >
                <label for="place_address">Distance from Any land mark: </label>
                <input type="text" class="form-control" id="place_address" name="bus_stop" value="{{$place->bus_stop}}" placeholder="Bus Stand 34 KM" maxlength="30" autocomplete="off" >
                <label for="place_address">Distance from any Popular Tourist Place:</label>
                <input type="text" class="form-control" id="place_address" name="other_place" value="{{$place->other_place}}" placeholder="St Pauls Catholic Church 34 KM" maxlength="35" autocomplete="off" >
               <!-- <label for="place_address">Distance from Metro Station:</label>
                <input type="text" class="form-control" id="place_address" name="metro_station" value="{{$place->metro_station}}" placeholder="Charbagh Metro Station 250 M" autocomplete="off" >
                <label for="place_address">Distance from Shopping Complex: *</label>
                <input type="text" class="form-control" id="place_address" name="shopping_complex" value="{{$place->shopping_complex}}" placeholder="Phoenix United Mal 45 KM" autocomplete="off" >-->
            </div>
        </div>
        ...
        <div id="contact_info">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="place_website" name="website" @if($place->website !="")value="{{$place->website}} @endif" placeholder="Enter website url" value="www.nsnhotels.com">
            </div>
        </div>
        ...
        <div id="media">
            ...
            <div class="row">
                <div class="col-md-6">
                    <p><strong>Hotel Front image:</strong></p>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <input type="file" class="form-control" id="gallery" name="banner" accept="image/*" multiple>
                </div>
            </div>
        ...
        <div id="golo_seo">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="seo_keywords" name="seo_keywords" value="">{{$place['seo_keywords']}}</textarea>
            </div>
            <div class="form-group">
              <label class="btn btn-secondary" data-toggle-class="btn-default" data-toggle-passive-class="btn-default">
                    <input type="date" name="o_u_s_to" value="{{$place['o_u_s_to']}}" class="join-btn">Room out of stock start
                </label>
                <!--</div>-->
                <!--<div class="form-group">-->
                <label class="btn btn-secondary" data-toggle-class="btn-default" data-toggle-passive-class="btn-default">
                    <input type="date" name="o_u_s_from" value="{{$place['o_u_s_from']}}" class="join-btn">Room out of stock end
                </label> 
                </div>
              ...
              <label class="btn btn-secondary" data-toggle-class="btn-default" data-toggle-passive-class="btn-default">
                  <label>Top Rated</label>
                   <select name ="top_rated" >
                       <option value="0"@if($place['top_rated']==0) selected @endif  >No</option>
                       <option value="1" @if($place['top_rated']==1) selected @endif>Yes</option>
                   </select>
                </label>
