                    ...
                    <div class="page-bar">
                        <div class="page-title-breadcrumb">
                            ...
                            <ol class="breadcrumb page-breadcrumb pull-right">
                                ...
                                <li><a class="parent-item" href="#">Price Calendar</a>&nbsp;</i>
                                </li>
                            </ol>
                        </div>
                    </div>
...
<input type="text" class="form-control digitsOnly valueEnter12 {{$cls}}" name="price_one_person[]" value="@if(isset($result) && ($result->date == $date_all_month) ){{trim($result->price_one_person)}}  @endif ">
                                                </div>
                                                 <div class="col-md-2 col-sm-2 col-xs-4">
                                                <label>Price Two Person</label>
<input type="numeric" name="price_two_person[]" class="form-control digitsOnly valueEnter21 {{$cls}}" value="@if(isset($result) && ($result->date == $date_all_month) ){{trim($result->price_two_person)}}  @endif
" autocomplete="off">
                                                </div>
                                                <div class="col-md-2 col-sm-2 col-xs-4">
                                                <label>Price Three Person</label>
<input type="text" name="price_three_person[]" class="form-control digitsOnly valueEnter31 {{$cls}}" value="@if(isset($result) && ($result->date == $date_all_month) ){{trim($result->price_three_person)}}  @endif "autocomplete="off"/>
                                                </div>
                                                 <div class="col-md-2 col-sm-2 col-xs-4">
                                                <label>Discount Percentage</label>
<input type="text" name="discount_percentage[]" class="form-control digitsOnly valueEnter41 {{$cls}}"  value="@if(isset($result) && ($result->date == $date_all_month) ){{trim($result->discount_percentage)}} @endif ">
                                                </div>
                                                @if(isset($result))
